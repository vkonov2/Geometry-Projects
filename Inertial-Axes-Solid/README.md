...
$J_u = (\cos{\alpha}, \cos{\beta}, \cos{\gamma})^T$, где $\overline{e}(\cos{\alpha},\cos{\beta},\cos{\gamma})$ - орт оси $u$, заданный в указанной системе координат своими направляющими косинусами, а симметрическая матрица $\displaystyle J = \begin{vmatrix} J_x & -J_{xy} & -J_{xz} \\ -J_{xy} & J_y & -J_{yz} \\ -J_{xz} & -J_{yz} & J_z \end{vmatrix}$ определяет **тензор инерции** тела (тензор второго ранга). 
